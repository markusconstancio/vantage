...
def base_css() -> str:
    """Shared stylesheet — for composing reports into a combined page."""
    return _CSS


def tooltip_js() -> str:
    """Shared tooltip layer script."""
    return _TOOLTIP_JS
...
def findings_card(recon_result: dict, analysis: dict) -> str:
    """The inner card HTML (hero + body + footer), reusable in a combined page."""
...
def render_findings_html(recon_result: dict, analysis: dict) -> str:
    target = analysis.get("target") or recon_result.get("target") or "unknown"
    return _page(f"Red Team Findings — {target}",
                 findings_card(recon_result, analysis))
...
def osint_card(assessment: dict) -> str:
    """The inner card HTML (hero + body + footer), reusable in a combined page."""
...
def render_osint_html(assessment: dict) -> str:
    return _page(f"OSINT Risk Assessment — {assessment['subject']}",
                 osint_card(assessment))
